...
def main(args) -> None:
    ...
    for gt, pred in product(
        ...
        for idx, (x, y) in enumerate(dl):
            ...
            # ImageDraw.Draw(img_draw).text(
            #     (10, 3 * SPACING), f"sample id: {idx}", TEXT_COLOR, FNT
            # )

            row = {
                "": img_draw,
                "sample": (x[0].permute(1, 2, 0) * 255).cpu().numpy().astype(np.uint8),
            }

            for composite in composite_list:
                with Gradient(model=model, composite=composite) as attributor:
                    y_hat, relevance = attributor(
    ...
    name = f"{args.model_path.parts[-3]}_{args.model_path.parts[-2]}.png"
    if args.short:
        name = "short_" + name
